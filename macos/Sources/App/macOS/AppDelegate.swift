...
extension AppDelegate {
    ...
    func pasteMenuShortcutMatches(_ event: NSEvent) -> Bool {
        guard let eventKey = MenuShortcutKey(event: event),
              let menuPaste,
              let pasteKey = MenuShortcutKey(
                  keyEquivalent: menuPaste.keyEquivalent,
                  modifiers: menuPaste.keyEquivalentModifierMask
              ) else {
            return false
        }

        return eventKey == pasteKey
    }
